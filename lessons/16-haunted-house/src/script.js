...
// Wall
const wallColorTexture = textureLoader.load('./wall/mixed_brick_wall_1k/mixed_brick_wall_diff_1k.webp')
const wallARMTexture = textureLoader.load('./wall/mixed_brick_wall_1k/mixed_brick_wall_arm_1k.webp')
const wallNormalTexture = textureLoader.load('./wall/mixed_brick_wall_1k/mixed_brick_wall_nor_gl_1k.webp')
...
roof.position.y += 2.5 + (1.5 * 0.5) // height of the wall + half the height of the cone
...
gui.add(graveMaterial, 'metalness').min(-1).max(1).step(0.01)
gui.add(graveMaterial, 'roughness').min(-1).max(1).step(0.01)

const graves = new THREE.Group()
scene.add(graves)

// When you send the same angle to sine and cosine, you end up with x and y coordinates of a circle positioning
...
for (let i = 0; i < 30; i++) {
    ...
    grave.position.x = x
    grave.position.y += Math.random() * 0.8 * 0.5
    grave.position.z = z
