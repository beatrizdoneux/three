...
sizes.resolution = new THREE.Vector2(sizes.width * sizes.pixelRatio, sizes.height * sizes.pixelRatio) // cannot be created at the same time as the sizes object because the key/values pairs don't exist yet
...
const createFirework = (count, position, size, texture, radius, color) => {
    ...
    const material = new THREE.ShaderMaterial({
        ...
        transparent: true,
        depthWrite: false,
        blending: THREE.AdditiveBlending, // color adds to what's behind
    })
...
const createRandomFirework = () => {
    const count = Math.round(400 + Math.random() * 1000) // random number from 400 - 1400
    const position = new THREE.Vector3(
        (Math.random() - 0.5) * 2,
        Math.random(),
        (Math.random() - 0.5) * 2,
